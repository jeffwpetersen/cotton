...
// Add Zen Tabs styles
if (theme_get_setting('kcbasic_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'kcbasic') .'/css/tabs.css');
}

function kcbasic_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . kcbasic_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = kcbasic_id_safe('page-'. $path);
    $vars['classes_array'][] = kcbasic_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('kcbasic_ie_enabled')) {
    $kcbasic_ie_enabled_versions = theme_get_setting('kcbasic_ie_enabled_versions');
    if (in_array('ie8', $kcbasic_ie_enabled_versions, TRUE)) {
...
function kcbasic_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from kcbasic_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function kcbasic_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function kcbasic_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('kcbasic_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('kcbasic_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('kcbasic_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('kcbasic_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('kcbasic_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function kcbasic_id_safe($string) {
 ...
 *
 */
function kcbasic_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = kcbasic_id_safe($element['#title']);
 ...
 */
function kcbasic_preprocess_menu_local_task(&$variables) {
 ...
 */
function kcbasic_menu_local_tasks(&$variables) {
  $output = '';
